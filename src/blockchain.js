...
class Blockchain {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (currentTime - messageTime >= FIVE_MINUTES_IN_SECONDS) {
          throw new Error("Message outdated");
        }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      const blockWIthMatchingHash = self.chain.find(
        (block) => block.hash == hash
      );
      if (!blockWIthMatchingHash) {
        reject(new Error("No matching blocks found"));
      }
      resolve(blockWIthMatchingHash);
    });
  }
